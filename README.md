...
- **📖 Learn from Reading** — search any topic and pick from two sources: **Wikipedia** (Simple English for A1/A2, full Wikipedia for B1+) or **The Guardian** (real journalism, B1+). The article renders with **at-your-level words underlined in green** and **stretch words in amber wavy underline**. Tap any highlight for a definition, native + TTS pronunciation, and an "I know it" shortcut that adds the word to your SRS queue. A vocabulary checklist at the bottom lets you tick known words; **mastery ≥ 90% on three articles in a row auto-promotes you to the next CEFR level**.
